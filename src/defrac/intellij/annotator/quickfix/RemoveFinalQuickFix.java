 ...
 *
 */
public final class RemoveFinalQuickFix extends BaseIntentionAction {
  @NotNull
  private final PsiMember member;

  public RemoveFinalQuickFix(@NotNull final PsiMember member) {
    this.member = member;
  }
        ...
        CommandProcessor.getInstance().executeCommand(member.getProject(), new Runnable() {
          public void run() {
            ApplicationManager.getApplication().runWriteAction(new Runnable() {
              @Override
              public void run() {
                setModifierProperty(member, PsiModifier.FINAL, false);
              }
            });
          }
